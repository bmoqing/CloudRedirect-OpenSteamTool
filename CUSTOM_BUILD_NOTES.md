...
[Selectively11/CloudRedirect](https://github.com/Selectively11/CloudRedirect) `v2.1.5`.

Target repository, when the owner explicitly approves publishing:

- `bmoqing/CloudRedirect-OpenSteamTool`

Do not push, tag, or create a GitHub release automatically. Publishing should be a separate
user-approved step, with a new iteration tag such as `v2.1.5-ost.1` rather than overwriting
an older release.
...
- Custom DLL source labeling:
  ...
  - hash mismatch
  - not installed
  - identifies custom builds by SHA256 and embedded custom-build marker
- Official DLL auto-update protection:
  - settings toggle is disabled for the custom DLL
  - first-run auto-update prompt writes `auto_update_dll=false` for custom builds
...
When upstream changes Steam hook support, keep upstream RVA/signature/vtable/API fixes first,
then reapply the localization, WebDAV, OpenSteamTool loader, diagnostics, restore, and packaging layer.
...
```powershell
dotnet publish ui\CloudRedirect.csproj -c Release -r win-x64 --self-contained false -o ui\bin\publish-v215
```
...
The app also detects the embedded custom-build marker, so a clean rebuild with a different hash
should still be shown as a custom WebDAV/OpenSteamTool build instead of an unknown DLL.

Verify outputs:

```powershell
Get-FileHash ui\bin\publish-v215\CloudRedirect.exe -Algorithm SHA256
...
```text
CloudRedirect.exe:      70BDB72B392CDE9BF432D4F062DE974CCCDB936356B005C56800B549115C0F17
cloud_redirect.dll:     1B59851D0C0674F1A87DC369F757A75A8B0F27C5A1E4CA696EC9439EDC0C0778
...
```powershell
dumpbin /exports ui\Resources\cloud_redirect.dll | findstr /C:CloudRedirectLua /C:CloudOnSendPkt /C:CR_InitCloudSave /C:CR_HandleCloudRpc
```
...
The workflow in `.github/workflows/release.yml` creates a release when a tag like `v2.1.5-ost.1`
is pushed. Push tags only after explicit approval from the repository owner.
...
Upstream updates:
- Fixed crashes with non-ASCII save paths.
- Fixed AutoCloud scans for Unicode install directories.
- Added initial third-party CloudRedirect API support.
- Added the first-run DLL auto-update prompt and skipped provider setup when Google Drive or OneDrive is already authenticated.

Custom build:
- OpenSteamTool integration through config/lua.
- Simplified Chinese localization.
- WebDAV provider and compatibility test.
- Diagnostics page, data restore page, DLL source labeling, and official-DLL auto-update protection.
```
